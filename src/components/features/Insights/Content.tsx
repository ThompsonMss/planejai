...
function SectionTitle({ children }: PropsWithChildren) {
  return (
    <h3 className="mt-5 mb-1.5 text-sm leading-relaxed font-semibold text-foreground">
      {children}
    </h3>
  );
}
    ...
    <div className="lg:max-h-93 lg:scrollbar-thin lg:[scrollbar-color:var(--border)_transparent] lg:overflow-y-auto lg:pr-2">
      <section className="flex flex-col gap-2">
        <div className="flex flex-col items-start gap-2 sm:flex-row">
          <span className="text-sm font-semibold text-foreground">🎯 Viabilidade da Meta</span>
          {status && (
            <span
